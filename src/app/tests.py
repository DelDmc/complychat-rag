'''Tests for the corpus downloader and the ingestion pipeline gate.

Everything here is stubbed at the HTTP boundary: no network access, no API
key, no fixtures on disk, and no dependency on the real sources CSV. The
tests follow the repo's unittest/Django convention and use SimpleTestCase
because none of the code under test touches the ORM.
'''
...
# The loader-side regression tests for citation metadata live on the
# test/citation-metadata branch and land here after PR #1 merges.
